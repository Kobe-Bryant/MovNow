...
-(void)initializeCameraWithPreview:(UIImageView *)preview
{
    ...
    if (_deviceAuthorized) {
        ...
        if (devices.count==0) {
            [self ShowAlertToUserWithTitle:NSLocalizedString(@"抱歉", nil) Message:NSLocalizedString(@"相机设备不可用", nil)];
...
- (void)checkDeviceAuthorizationStatus
{
   if ([AVCaptureDevice authorizationStatusForMediaType:AVMediaTypeVideo]==AVAuthorizationStatusDenied) {
        ...
        [self ShowAlertToUserWithTitle:NSLocalizedString(@"无法启动相机", nil)  Message:NSLocalizedString(@"\n请为应用开放相机权限:设置-隐私-相机-Kept(打开)",nil)];
...
-(void)SetCaptureSessionWithCamera:(enum KCameraChose)chose
{
    ...
    AVCaptureDeviceInput *input = [AVCaptureDeviceInput deviceInputWithDevice:current_camera error:&error];
        if (!input) {
        [self ShowAlertToUserWithTitle:NSLocalizedString(@"提示", nil) Message:NSLocalizedString(@"无法连接到相机", nil)];
        }
    //首次添加输入端口
...
- (void)CaptureImage {
    ...
    [_stillImageOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler: ^(CMSampleBufferRef imageSampleBuffer, NSError *error) {
        ...
        if (imageSampleBuffer != NULL) {
            ...
            [self.CameraDelegate didFinishPickPhotoImage:[UIImage imageWithData:imageData]];
            imageData = nil;
        }
    }];
    }
...
#pragma mark--提示用户
-(void)ShowAlertToUserWithTitle:(NSString *)title Message:(NSString*)msg
{
    UIAlertView *alert=[[UIAlertView alloc]initWithTitle:nil message:nil delegate:nil cancelButtonTitle:NSLocalizedString(@"确定", nil) otherButtonTitles: nil];
